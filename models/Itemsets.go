...
type Itemsets struct {
	...
	Weight      int       `orm:"column(weight);type(int)" json:"weight"`
...
func (item *Itemsets) FindAll() (list []*Itemsets, totalCount int, err error) {
	_, err = item.QueryTable().OrderBy("weight").All(&list)
	if err != nil {
		return
	}
...
func (item *Itemsets) FindToPager(pageIndex, pageSize int) (list []*Itemsets, totalCount int, err error) {
	...
	_, err = item.QueryTable().OrderBy("weight").Offset(offset).Limit(pageSize).All(&list)
	if err != nil {
		return
	}
