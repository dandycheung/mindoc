...
func (book *Book) ThoroughDeleteBook(id int) error {
	...
	// 删除最近浏览记录
	err = NewBookPin().DeleteByBookId(book.BookId)
	if err != nil {
		o.Rollback()
		return err
	}
	...
	// 删除 Pin
	err = NewBookRecord().DeleteByBookId(book.BookId)
	if err != nil {
		o.Rollback()
		return err
	}
...
func (book *Book) ImportBook(zipPath string) error {
	...
	w := md5.New()
	io.WriteString(w, zipPath) // 将 str 写入到 w 中
	...
	err := filepath.Walk(tempPath, func(path string, info os.FileInfo, err error) error {
		...
		if !info.IsDir() {
				...
				doc.Markdown = linkRegexp.ReplaceAllStringFunc(doc.Markdown, func(link string) string {
					...
					if err == nil {
						...
						if filetil.FileExists(linkPath) {
							...
							// 如果链接是 Markdown 文件，则生成文档标识，否则，将目标文件复制到项目目录
							if strings.EqualFold(ext, ".md") || strings.EqualFold(ext, ".markdown") {
			...
			// 如果当前目录下存在 Markdown 文件，则需要创建此节点
...
func (book *Book) FindByIds(bookIds []int) ([]*Book, error) {
	o := orm.NewOrm()
	var books []*Book

	if len(bookIds) > 0 {
		_, err := o.QueryTable(book.TableNameWithPrefix()).Filter("book_id__in", bookIds).All(&books)
		return books, err
	} else {
		return books, nil
	}
...
func (book *Book) FindForHomeByIds(bookIds []int, memberId, offset, limit int) (books []*BookResult, err error) {
	// bookIds = append(bookIds, 10000, 10001, 10002)
	// orm.Debug = true

	o := orm.NewOrm()
	sql := `SELECT book.*,rel1.*,member.account AS create_name,member.real_name FROM md_books AS book
	LEFT JOIN md_relationship AS rel ON rel.book_id = book.book_id AND rel.member_id = ?
	left join (select book_id,min(role_id) AS role_id
			   from (select book_id,role_id
					 from md_team_relationship as mtr
  ...
  as t group by book_id) as team on team.book_id=book.book_id
	LEFT JOIN md_relationship AS rel1 ON rel1.book_id = book.book_id AND rel1.role_id = 0
	LEFT JOIN md_members AS member ON rel1.member_id = member.member_id
  WHERE book.book_id IN (?) AND book.privately_owned = 0 or rel.role_id >=0 or team.role_id >=0 ORDER BY order_index desc,book.book_id DESC LIMIT ?,?`

	_, err = o.Raw(sql, memberId, memberId, bookIds, offset, limit).QueryRows(&books)
	return
}
